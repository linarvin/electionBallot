...
User Instructions:
Run the electionBallot.py file using the following command:
python electionBallot.py 

1. Input your First Name, Last Name, 9 Digit ID Number (Does not have to be real), and the candidate of your choice. The output file will be voteCount_PlainText.txt which holds the votes.
...
2. Click on voteCount_Encrypted.aes-128-cbc.txt to check if the voteCount_PlainText.txt is encrypted (and it should be). 
Run the line in the terminal:
...
After decrypting, voteCount_PlainText.txt and voteCount_Decrypted.txt should hold the same information.
For extra security, the user may use a hash to confirm if a text file has been edited.
...
These lines should return the same hash value. If someone tampers with either text file, then the hashes will no longer match and the user will know if someone has interfered with the results.
